...
def run(plot=True):
    ...
    for input_dict in settings.list_of_input_dicts:
        ...
        fig = plot_tc_curves(grt_temperature_vector, sample_res_vector, fig, input_dict)
    if plot:
...
def normalize_squid_output(sample_res_vector, input_dict):
...
def load_data(input_dict, quick_plot=False):
...
def resistance_to_temp_grt(grt_res_vector, serial_number):
    grt_temperature_vector = resistance_to_temp(grt_res_vector, serial_number)
    return grt_temperature_vector


def plot_tc_curves(grt_temperature_vector, sample_res_vector, fig, input_dict):
    if fig is None:
