...
def load_beam_map_data(data_path, map_limit=2.5e6):
    '''
...
def parse_beam_map_data(data_dict, source_distance=18.0):
    '''
    ...
    if len(np.arange(-boundary_x, boundary_x + step_size_x, step_size_x)) % 2 == 0:
        ...
        X, Y = np.meshgrid(np.arange(-boundary_x, boundary_x + step_size_x, step_size_x),
                           np.arange(-boundary_x, boundary_x + step_size_x, step_size_x))
    ...
    initial_guess = guess_fit_params(raw_amplitude.reshape(X.shape))
    fit_params = fit_function(twoD_Gaussian, X, Y, raw_amplitude, initial_guess)
    print fit_params[3], fit_params[4], fit_params[3] / fit_params[4]
    fit_amplitude = twoD_Gaussian((X, Y), *fit_params)
...
def plot_beam_map(processed_data_dict, frequency='220',
    ...
    Z_data, Z_fit, Z_res = processed_data_dict['normalized_amplitude'], processed_data_dict['fit_amplitude'],\
    ...
    if plot_raw or save_raw:
        fig_raw = plt.figure(figsize=(6.5, 6.5))
        box_size = 0.72
        box_start_corner = 0.14
        ...
        #ax_raw.set_xlim((-9.5, 9.))
        #ax_raw.set_ylim((-9.5, 9.))
    ...
    if plot_fits:
        ...
        bot_position = 0.08
        cb_width = 0.06
        fig_fits = plt.figure(figsize=(fig_width, 2.0 * fig_width))
        ...
        cb_top_height = top_position + sub_plot_height - mid_position
        # Add the axes for the plots
        fit_data_cb_axis = fig_fits.add_axes((cb_align_position, mid_position, cb_width, cb_top_height))
        res_cb_axis = fig_fits.add_axes((cb_align_position, bot_position, cb_width, cb_bot_height))
        # Add the axes for the plots
        ax_fits_data = fig_fits.add_axes((left_align_position, top_position, sub_plot_width, sub_plot_height))
        ax_fits_fit = fig_fits.add_axes((left_align_position, mid_position, sub_plot_width, sub_plot_height))
        ax_fits_res = fig_fits.add_axes((left_align_position, bot_position, sub_plot_width, sub_plot_height))
        # Create the color map
        color_plot_data = ax_fits_data.pcolor(X, Y, Z_data, label='Data',
                                              vmin=0, vmax=np.nanmax(Z_data),
                                              cmap=matplotlib.cm.jet)
        contour_plot_data = ax_fits_data.contour(X, Y, Z_data, 8, label='Fit', colors='w',
                                                 vmin=0, vmax=np.nanmax(Z_data))
        color_plot_fit = ax_fits_fit.pcolor(X, Y, Z_fit, label='Fit',
                                            vmin=np.nanmin(Z_data), vmax=np.nanmax(Z_data),
                                            cmap=matplotlib.cm.jet)
        contour_plot_fit = ax_fits_fit.contour(X, Y, Z_fit, 8, label='Fit', color='w',
                                               vmin=np.nanmin(Z_data), vmax=np.nanmax(Z_data))
        color_plot_res = ax_fits_res.pcolor(X, Y, Z_res, label='Residual',
                                            vmin=np.nanmin(Z_res), vmax=np.nanmax(Z_res),
                                            cmap=matplotlib.cm.jet)
        contour_plot_res = ax_fits_res.contour(X, Y, Z_res, 8, label='Fit', color='w',
                                               vmin=np.nanmin(Z_data), vmax=np.nanmax(Z_data))
        # Add text labels to the color map
        ax_fits_data.text(0.05, 0.85, 'Data', color='w', fontsize=16, transform=ax_fits_data.transAxes)
        ax_fits_fit.text(0.05, 0.85, 'Fit', color='w', fontsize=16, transform=ax_fits_fit.transAxes)
        ax_fits_res.text(0.05, 0.85, 'Residual', color='k', fontsize=16, transform=ax_fits_res.transAxes)
        if clip:
            # Data
            ax_fits_data.set_xlim([-clip_value, clip_value])
            ax_fits_data.set_ylim([-clip_value, clip_value])
            # Fit
            ax_fits_fit.set_xlim([-clip_value, clip_value])
            ax_fits_fit.set_ylim([-clip_value, clip_value])
            # Residual
            ax_fits_res.set_xlim([-clip_value, clip_value])
            ax_fits_res.set_ylim([-clip_value, clip_value])
        # Add the color bar
        fig_fits.colorbar(color_plot_data, cax=fit_data_cb_axis)
        fig_fits.colorbar(color_plot_res, cax=res_cb_axis)
        # Basic Plotting Options 
        ax_fits_data.set_xlabel('X position ($^{\circ}$)', fontsize=16)
        ax_fits_fit.set_xlabel('X position ($^{\circ}$)', fontsize=16)
        ax_fits_res.set_xlabel('X position ($^{\circ}$)', fontsize=16)
        ax_fits_data.set_ylabel('Y position ($^{\circ}$)', fontsize=16)
        ax_fits_fit.set_ylabel('Y position ($^{\circ}$)', fontsize=16)
        ax_fits_res.set_ylabel('Y position ($^{\circ}$)', fontsize=16)
        ax_fits_data.set_title('Beam Map {0} GHz'.format(frequency))
        save_str_fits = './Output/Python/Beam_Map_{0}_GHz_fits.png'.format(frequency)
        if save_fits:
            fig_fits.savefig(save_str_fits, figsize=(6,6))
        fig_fits.show()
    _ask_user_if_they_want_to_quit()
    if plot_3d:
        fig_3d = plt.figure()
        ...
        ax_3d.set_title('280 GHz 3D Beammap')
...
def twoD_Gaussian((x, y), amplitude, x_0, y_0, sigma_x, sigma_y, theta):
    x_0 = float(x_0)
    y_0 = float(y_0)
...
def fit_function(function, X, Y, data, initial_guess):
    popt, pcov = opt.curve_fit(twoD_Gaussian, (X, Y), data, p0=initial_guess)
    return popt


def guess_fit_params(data):
    height = data.max()
    x = 0
    y = 0
    width_x = 1.0
...
def run(data_path, frequency, map_limit=2.5e6, test=False):
    if test:
        X = np.linspace(0, 200, 201)
        Y = np.linspace(0, 200, 201)
        X, Y = np.meshgrid(X, Y)
        initial_params = (3, 100, 100, 20, 40, -45)
        data = twoD_Gaussian((X,Y), *initial_params)
        data_with_noise = data + 0.2 * np.random.normal(size=data.shape)
        initial_guess = (3, 100, 100, 20, 40, 0)
        fit_params = fit_function(twoD_Gaussian, X, Y, data_with_noise, initial_guess)
        fitted_data = twoD_Gaussian((X, Y), *fit_params)
        fig, ax = plt.subplots(1, 1)
        ax.hold(True)
        ax.pcolor(X, Y, fitted_data.reshape(X.shape), cmap=plt.cm.jet)
        ax.contour(X, Y, fitted_data.reshape(X.shape), 8, colors='w')
        fig.show()
        _ask_user_if_they_want_to_quit()
    data_dict = load_beam_map_data(data_path, map_limit)
    processed_data_dict = parse_beam_map_data(data_dict, source_distance=18.0)
    fig = plot_beam_map(processed_data_dict, frequency, clip=False, clip_value=12)
